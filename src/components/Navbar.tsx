...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'bg-dark-gray bg-opacity-90 backdrop-blur-md shadow-md' : 'bg-transparent'
    }`}>
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden bg-dark-gray bg-opacity-95 overflow-hidden transition-all duration-300 ease-in-out ${
        isOpen ? 'max-h-screen py-4' : 'max-h-0'
      }`}>
        <div className="container mx-auto px-4 flex flex-col space-y-4">
          <button
            onClick={() => scrollToSection('ventajas')}
            className="text-white py-2 hover:text-key-green transition-colors"
          >
            Ventajas
          </button>
          <button
